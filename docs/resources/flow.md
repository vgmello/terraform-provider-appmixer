...
page_title: "appmixer_flow Resource - terraform-provider-appmixer"
subcategory: ""
description: |-
  Manages an Appmixer flow. The flow descriptor is stored in flow_json and compared on every plan; volatile server-owned fields (stage, timestamps, userId) and node component version fields (rewritten by the server to the tenant's installed version on save) are excluded from drift detection.
