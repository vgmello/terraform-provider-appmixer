...
The whole of `docs/` is generated by [`tfplugindocs`](https://github.com/hashicorp/terraform-plugin-docs) from the provider schemas, `examples/`, and the hand-written pages in `templates/`. After changing a schema, an example, or a guide, regenerate with:

```bash
make docs
```

Run it through `make`, not `tfplugindocs` directly: the target passes the provider-name flags that keep page titles as `appmixer …`, and a bare invocation rewrites every page. Anything that exists only under `docs/` is deleted on regeneration, so hand-written pages belong in `templates/` — see `templates/guides/`. `make docs-check` regenerates and fails if the committed output is stale; CI runs it on every PR.
