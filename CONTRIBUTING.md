...
- **Mock server handlers must not echo request bodies verbatim** where the real API rewrites them. A permissive mock hides the bug class above. `POST`/`PUT /flows` upgrades component versions on purpose (`upgradeComponentVersions` in `internal/mockserver/routes_flows.go`) — mirror any comparable rewrite you learn about from the real API.
